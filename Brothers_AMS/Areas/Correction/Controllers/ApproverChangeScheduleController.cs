    ...
    public class ApproverChangeScheduleController : Controller
    {
        ...
        public ActionResult ApproverChangeSchedule(string RefNo, string CSType, string Approved)
        {
            ...
            if ((RefNo != null && CSType != null) || Approved != null)
            {
                ...
                if (result.Count > 0)
                {
                    return Redirect("http://apbiphap05:2020/Correction/ApproverChangeSchedule/ApproverChangeSchedule?Approved=" + result[0]);
        ...
        public ActionResult GetApproverCSList()
        {
            ...
            List<GET_AF_CSRequest_Result> list = db.GET_AF_CSRequest().ToList();
        ...
        public ActionResult ApprovedCS(List<AF_CSModel> GetApproved, string ifalter)
        {
            ...
            foreach (AF_CSModel csrequest in GetApproved)
            {
                ...
                csfile = (from c in db.AF_ChangeSchedulefiling where c.CS_RefNo == csrequest.CS_RefNo && c.EmployeeNo == csrequest.EmployeeNo select c).FirstOrDefault();
                ...
                if (csfile.Status > 0)
                {
                    db.AF_EmailCSRequest(csfile.CS_RefNo);
                }
                db.SaveChanges();
            }
