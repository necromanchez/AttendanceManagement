...
function Initializepage() {
        ...
        columns: [
            ...
            
            {
                title: "Action", data: function (x) {
                    var logO = (x.Logo == "") ? "no-logop.png" : x.Logo;
                    return "<button type='button' class='btn btn-sm' style='background-color:#039a8c; color:white' alt='alert' class='model_img img-fluid' id=logo" + x.ID + " onclick=idlogof('logo" + x.ID + "','" + logO + "')>" +
    ...
    $('#AgencyTable tbody').on('click', '.btndelete', function () {
        ...
        var data = tabledata.row($(this).parents('tr')).data();
        $('#ID').val(data.ID);
        Deletionheres('../Agency/DeleteAgency', data.ID, data.Agency);
                ...
                "<div class='col-md-2'>"+
                    "<div class='form-group'>"+
                        "<div class='input-group outut'>" +
                            "<span class='input-group-addon' style='border-color:white;'>Email "+ID+ "</span>" +
                        "</div>"+
                    "</div>"+
                "</div>"+
                ...
                "<div class='col-md-9'>"+
                    "<div class='form-group'>"+
                        "<div class='input-group'>"+
                            "<input type='email' class='form-control Emailinput' id='EmailA" +ID + "' name='SupervisorName' />" +
                        "</div>"+
                    "</div>"+
                "</div>" +
                 "<div class='col-md-1'>"+
